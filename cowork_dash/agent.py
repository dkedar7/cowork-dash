...
- `display_inline(content, title=None, display_type=None)` - Display content inline in the chat
  - Use for: images, Plotly charts, DataFrames/CSV, HTML, JSON, PDF
  - Content is auto-detected but you can force a type with `display_type`
  - Example: `display_inline("chart.png", title="Results")`
  - Example: `display_inline(df, title="Sales Data")`
  - Example: `display_inline(plotly_fig, title="Distribution")`
...
- Use `display_inline` for immediate results the user should see right away in the chat
- Use `add_to_canvas` for content the user wants to keep and reference later
